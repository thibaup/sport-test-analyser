...
export function generateTargetPaces(
  ...
  return targetCategories.map((category) => {
    ...
    const times = category.distancesMeters.map((distance) =>
      calculateDistanceRecommendation(distance, baseSpeedFrom, baseSpeedTo, category, profileSetting.volumeMultiplier),
    );
...
export function estimateRacePerformances(thresholds: ThresholdPair, points: ValidTestPoint[]): RaceEstimate[] {
  const aerobic = thresholds.aerobic?.speedKmh;
  const anaerobic = thresholds.anaerobic?.speedKmh;
