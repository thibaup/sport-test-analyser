...
import type { AppAnalysis, AthleteInfo, DistanceUnit, PaceUnit, TestStep, ZoneProfile } from '../types/lactate';
...
export function ExportPanel({ athleteInfo, steps, analysis, paceUnit, distanceUnit, profile, language }: ExportPanelProps) {
  const [status, setStatus] = useState('');

  const exportJson = () => {
    downloadFile(
      'lactate-analysis.json',
      JSON.stringify({ athleteInfo, steps, results: publicResults(analysis) }, null, 2),
      'application/json',
    );
  ...
  const exportCsv = () => {
    downloadFile('lactate-analysis.csv', toCsv(steps, analysis), 'text/csv');
  ...
  const exportPdf = async () => {
    await exportPdfReport({ athleteInfo, steps, analysis, paceUnit, distanceUnit, profile, language });
  ...
  return (
    <section id="export" className="section-card">
      ...
      {status && <p className="mt-4 rounded-lg bg-emerald-50 px-4 py-2 text-sm font-medium text-emerald-700">{status}</p>}
    </section>
  );
}
...
function toCsv(steps: TestStep[], analysis: AppAnalysis): string {
  const rows = [
    ['section', 'step', 'distance_km', 'speed_kmh', 'pace_sec_per_km', 'duration_sec', 'lactate', 'heart_rate', 'rpe', 'note'],
    ...steps.map((step) => [
      'input',
      step.step,
      step.distanceKm ?? '',
...
function buildSummary(athleteInfo: AthleteInfo, analysis: AppAnalysis, paceUnit: PaceUnit, language: Language): string {
...
function publicResults(analysis: AppAnalysis) {
  return {
    ...
    targets: analysis.targets.map((target) => ({
      ...
      speedFromKmh: target.speedFromKmh,
      speedToKmh: target.speedToKmh,
...
function stripThreshold(threshold: AppAnalysis['selectedThresholds']['aerobic']) {
